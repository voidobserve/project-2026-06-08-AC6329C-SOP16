...
extern u8 full_color_circulate_cnt;

u8 open_flag = 1;
void io_key_handle(struct sys_event *event)
{
    u8 event_type = 0;
    u8 key_value = 0;
    event_type = event->u.key.event; //按键的事件，单击，长按，双击等等
    key_value = event->u.key.value; //按键值

    static u16 timeout = 0;
    printf("io_key_handle");
    printf("\n event_type = %d",event_type);
    printf("\n event->u.key.type = %d",event->u.key.type);
    printf("key_value = %d",key_value);

    if(event->u.key.type == KEY_DRIVER_TYPE_IO)
    {


        if(key_value == 0)    
        {
            if(event_type == KEY_EVENT_HOLD && gpio_read(IO_PORTB_05) == 0 )   //长按遥控器电源键
            {
                 ...
                 {
                    timeout = 0;
                    if(get_on_off_state() == DEVICE_ON && open_flag == 1 )
                    {
            ...
            if(event_type == KEY_EVENT_CLICK && get_on_off_state() == DEVICE_ON )
            {
                extern void full_color_circulate(void);
                full_color_circulate();
            }
        }
        ...
        if(key_value == 1)
        {
            if(event_type == KEY_EVENT_CLICK  && get_on_off_state() == DEVICE_ON )
            {
                ...
                extern  void fc_bright_plus(void);
                fc_bright_plus();
...
{
    if(gpio_read(IO_PORTB_05) == 1 && only_one == 1)
    {
    ...
    if(gpio_read(IO_PORTB_05) == 0 && only_one == 0)
    {
